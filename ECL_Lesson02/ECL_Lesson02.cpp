...
Mat CvtCub2Sph(Mat* cube, Mat* original, Camera* Player, Mat* CameraViewer) {
    /*
    ...
    */
    ...
    yIndex = 0;
    for (int j = Player->pitch; j < (Player->heightPixel) + Player->pitch; j++)
    {
        /*
        ...
        */
        ...
        theta = v * PI;
        xIndex = 0;
        for (int i = Player->yaw; i < (Player->widthPixel) + Player->yaw; i++)
        {
            ...
            */
            ...
            CameraViewer->at<Vec3b>(yIndex, xIndex++) = cube->at<Vec3b>(yPixel, xPixel);
        }
        yIndex++;
    }
...
int main(int ac, char** av) {
    ...
    Mat CameraViewer = Mat::zeros(player.heightPixel, player.widthPixel, img.type());
    ...
    while (true) {
        ...
        {
            player.yaw -= 5;
            CubemapToSpherical = CvtCub2Sph(&SphericalToCubemap, &img, &player, &CameraViewer);

        }
        else if (keycode == 0x270000 && player.yaw + player.widthPixel + 5 < img.size().width) // yaw right
        {
            player.yaw += 5;
            CubemapToSpherical = CvtCub2Sph(&SphericalToCubemap, &img, &player, &CameraViewer);
        }
        else if (keycode == 0x260000 && player.pitch - 5 > 0) // pitch down
        {
            player.pitch -= 5;
            CubemapToSpherical = CvtCub2Sph(&SphericalToCubemap, &img, &player, &CameraViewer);
        }
        else if (keycode == 0x280000 && player.pitch + player.heightPixel + 5 < img.size().height) // pitch up
        {
            player.pitch += 5;
            CubemapToSpherical = CvtCub2Sph(&SphericalToCubemap, &img, &player, &CameraViewer);
        }
